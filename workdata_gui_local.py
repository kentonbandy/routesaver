...
pull_url = 'https://routesaver.app/pull/'
push_url = 'https://routesaver.app/push/'
script_dir = os.path.dirname(__file__)
db_file = Path(f'{script_dir}/workdata_database.json')

def startup():
    change_frame(login_frame)
    ...
    for p, d, files in os.walk(f'{script_dir}/workdata_backup/'):
        if files:
            try:
                ...
                x = backup[einvar.get()]['workdata']
                y = backup[einvar.get()]['default_salary']
            except KeyError:
                backup = None
        else:
            backup = None
...
def get_data():
    ...
    data = data[einvar.get()]
    try:
        create_base_entry()
    except TypeError:
        fail_var.set("Could not create base entry: TypeError")
...
def new_db_file():
    empty_db_file = {einvar.get(): {'default_salary': 0, 'workdata': []}}
    ...
    if Path(f'{script_dir}/workdata_database.json').is_file():
        success_mm_button.config(command=lambda: [get_data(), check_salary()])
        success_var.set("New database file created!")
        change_frame(success_frame)
    else:
        fail_var.set("Unable to create database file")
        change_frame(fail_frame)
...
def add_ein_to_db(db):
    ein_db = {}
    ein_db[einvar.get()] = db
    return ein_db
...
def set_salary(num):
    ...
    ein_db = add_ein_to_db(data)
    db_file.write_text(json.dumps(ein_db, indent=2))
    get_data()
...
def check_db(new_db):
    ...
    if new_db == data:
        ...
        ERROR_MSG = "Database update failed cross-check"
        change_frame(fail_frame)
...
def login_tracer(input_, ent, lim):
    inp = input_.get()
    out = ""
    for c in inp:
        try:
...
def date_format(input_, ent, but=None):
    global lastinp
    try:
        len(lastinp)
    except NameError:
    ...
    if len(inp) >= 10:
        if but:
            cs_dateentry.config(bg=green)
            but.config(state=NORMAL)
    else:
        if but:
            cs_dateentry.config(bg='yellow')
...
def cs_frame_config_1():
    ...
    cs_button_1.config(state=NORMAL, command=lambda: [set_salary(int(cs_salentry_var.get())), cs_dateentry.config(bg='yellow'), cs_frame_config_2(), cs_salentry_var.set("")])
...
def cs_frame_config_2():
    ...
    cs_dateentry_var.trace('w', lambda *args: date_format(cs_dateentry_var, cs_dateentry, cs_button_1))
    ...
    cs_button_1.config(state=DISABLED, command=lambda: [backdate(cs_dateentry_var.get()), cs_dateentry_var.set("")])
    cs_button_2.config(command=lambda: [change_frame(mm_frame), cs_dateentry_var.set("")])

frame_history = []

# login_frame population
einvar = StringVar()
einvar.trace('w', lambda *args: login_tracer(einvar, login_entry, 8))
login_title = Label(login_frame, text="Enter your EIN\n(Found on your paystub as \"EMPLOYEE ID\")", bg=bgc, fg=fgc)
login_entry = Entry(login_frame, textvariable=einvar, width=10, bg='yellow', justify='center')
login_button = Button(login_frame, text="Log in", state=DISABLED, command=init_db_check)
exit_button = Button(login_frame, text="Exit", state=NORMAL, command=quit)
